...
class OpenAIProvider:
    """Provider implementation for OpenAI's chat completions API using raw httpx."""
    ...
    async def count_tokens(self, request: ProviderRequest) -> TokenCount:
        # OpenAI does not have a dedicated token counting endpoint.
        # Estimate as len(json.dumps(payload)) // 4.
        body = _build_body(request, stream=False)
...
def _convert_messages(
    ...
    for msg in messages:
        ...
        if msg.role == "user":
            ...
            if other_content:
                content_parts = _convert_user_content(other_content)
...
def _convert_user_content(content: list[Content]) -> list[dict[str, Any]]:
    parts: list[dict[str, Any]] = []
    for c in content:
        if isinstance(c, TextContent):
            parts.append({"type": "text", "text": c.text})
        elif isinstance(c, MediaContent):
            if c.mime_type.startswith("image/"):
                parts.append({
                    "type": "image_url",
                    "image_url": {"url": f"data:{c.mime_type};base64,{c.data}"},
                })
            elif c.mime_type.startswith("audio/"):
                parts.append({
                    "type": "input_audio",
                    "input_audio": {
                        "data": c.data,
                        "format": c.mime_type.split("/")[-1],
                    },
                })
    return parts
...
def _check_status(resp: httpx.Response) -> None:
    ...
    if resp.status_code == 429:
        ...
        raise RateLimitError("openai", retry_after_ms=retry_ms)

    try:
        body = resp.text
    except httpx.ResponseNotRead:
        body = ""
    raise APIError("openai", resp.status_code, body)
